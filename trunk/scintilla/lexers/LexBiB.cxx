		...
		int length, int initStyle, WordList *keywordlists[], Accessor &styler) 
{
    ...
    for(; going; sc.Forward()){
        ...
        switch(sc.state){
            case SCE_BIB_RECORD:
                if(!isWordCharacter(sc.ch)) sc.SetState(SCE_BIB_DEFAULT);                    
            break;
            case SCE_BIB_FIELD:
                if(!isWordCharacter(sc.ch)) {  //sc.SetState(SCE_BIB_DEFAULT);                    
            ...
            break;
            case SCE_BIB_TEXT:
                if(!isWordCharacter(sc.ch) || !isspace(sc.ch)) sc.SetState(SCE_BIB_DEFAULT);
            break; 
        ...
        if(sc.state==SCE_BIB_DEFAULT){
            ...
            else if(afterEqual==0 && isWordCharacter(sc.ch)) sc.SetState(SCE_BIB_FIELD);
