...
--~ (tostring(props['PLAT_GTK'])=='1')

function do_set_menu()
	--[mhb] 06/21/09 commented: to skip setting debug menu
	--[[
	scite_Command {
	  'Step|do_step|Alt+C',
...
--[mhb] 06/21/09 commented: no need to set these menu
--~ scite_Command {
...
function Dbg:parameter_string()
    ...
    local parms = ' '
	local i=3 --[mhb] revised since $1 and $2 are reserved: i=1
    local parm = props[i]
    while parm ~= '' do
        if find(parm,'%s') then
...
--	spawner.fulllines(1)	
	...
	--return spawner_obj:run()    --[mhb] commented 
	
	--[mhb] added 04/23/09: to automatically make the console window staying on top
	local res=spawner_obj:run() 
	local s_cmd=props['CMD_WIN_ON_TOP'] or ''
	if string.len(s_cmd) then
		print('Running $(CMD_WIN_ON_TOP): ',s_cmd)
		os.execute(s_cmd)
	end
	return res
end
...
--~ 	if GTK then return end --[mhb] added 06/21/09
--~ 	spawner.foreground() --[mhb] 06/21/09 uncommented
end
