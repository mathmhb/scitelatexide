...
function find_ctag(f,partial)
  ...
  if result ~= gTagFile then
	...
	if not tags then --[mhb] added: to detect error setting of ctags.path.cxx
		print("No tags found in file "..gTagFile..'!')
		print("Please set ctags.path.cxx to full path of tags file!")
		return
	end
  end
  if partial then
