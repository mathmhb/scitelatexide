...
void SciTEBase::SetTextProperties(
	...
	std::string mo = GUI::UTF8FromString(localiser.Text("MONO"));
...
void SciTEBase::UpdateStatusBar(bool bUpdateSlowData) {
	if (sbVisible) {
		...
		propsStatus.SetInteger("FoldLevel",
		        wEditor.Call(SCI_GETFOLDLEVEL, GetCurrentLineNumber()) - SC_FOLDLEVELBASE);
...
//!-end-[fix_invalid_codepage]
	...
	case IDM_SET_MAINFILE:
		if (filePath.IsSet()) {
			bool is_mainfile=filePath.SameNameAs(mainFilePath.AsInternal());
			is_mainfile=!is_mainfile;
			mainFilePath = is_mainfile ? filePath : GUI_TEXT("");
			props.Set("MainFile", mainFilePath.AsUTF8().c_str()); 
	...
	case IDM_COMPILE_MAINFILE:
		if (SaveIfUnsureForBuilt() != IDCANCEL) {
			if (!mainFilePath.IsSet()) {
				...
				SelectionIntoProperties();
				const SString do_cmd=props.GetWild("command.compilemain.", f.AsUTF8().c_str());
				const SString do_dir=props.GetNewExpand("command.compilemain.directory.", f.AsUTF8().c_str());
				const SString do_subsys=props.GetNewExpand("command.compilemain.subsystem.", f.AsUTF8().c_str());
				if (!do_cmd.length()) {
...
//!-end-[GoMessageImprovement]
	...
	case SCN_UPDATEUI:
		...
		{
			int pos=wEditor.Call(SCI_GETCURRENTPOS);
			int charcode=wEditor.Call(SCI_GETCHARAT,pos>0?pos-1:0);
			if (charcode>=0) {
				char buf[100];
				sprintf(buf,"%1c",charcode);
...
//!-start-[LangMenuChecker]
	...
	if(language != last_lang) {
		...
		//~ WindowMessageBox(wSciTE, GUI::StringFromUTF8(language.c_str()).c_str(), MB_OK | MB_ICONWARNING);
		for (int item = 0; item < languageItems; item++) {
			...
			//[mhb] 12/31/11 revised to use property 'FileType' directly so as to fix language menu display bug 
			//~ if(language == languageMenu[item].extension && language!="") {
				//~ WindowMessageBox(wSciTE, GUI::StringFromUTF8(props.GetNewExpand("lexer.", fn.c_str()).c_str()).c_str(), MB_OK | MB_ICONWARNING);
